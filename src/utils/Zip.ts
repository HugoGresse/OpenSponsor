...
export class Zip {
    ...
    async fetch2Zip(
        ...
        for (const file of filesArray) {
            ...
            const uint = [...new Uint8Array(buffer)] as unknown as Uint8Array & {
                modTime: string
                fileUrl: string
            }
    ...
    makeZip(): void {
        ...
        for (const name in zip) {
            ...
            const size = this.reverse(zip[name].length.toString(16).padStart(8, '0'))
            const nameFile = this.str2hex(zip[name].fileUrl).join(' ')
            const nameBytes = new TextEncoder().encode(zip[name].fileUrl)
